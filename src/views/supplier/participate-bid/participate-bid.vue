<template>
	<div class="zeco-participate-bid">
		<section class="participate-bid">
			<ul v-infinite-scroll="loadMore()" infinite-scroll-disabled="loading" infinite-scroll-distance="50">
				<li class="list-box-content" v-for="listItem in list" v-link="{name: 'Bid', params: {bid_no: listItem.bidId}}">
					<box-section :head-title="listItem.title" :sub-title="listItem.companyName">
						<img slot="img" :src="listItem.logo ? listItem.logo : ''">
	...
	export default {
		...
		methods: {
			loadMore () {
				...
				this.$http.get(apis.urls.myBid, options).then((response) => {
					let listTpl = apis.pures.pureMyBidList(response.body);
...
<style lang="scss">
	...
	// 修正列表项背景色和底部线
	.zeco-participate-bid .participate-bid{
		background-color: $color-white;
	}
	.zeco-participate-bid ul{
		@include border-bottom($border-color);
	}
	.zeco-participate-bid ul>li:not(:last-child){
		@include border-bottom($border-color);
	}
	.zeco-participate-bid ul>li:last-child .box-section:before{
		border-bottom: 0;
	}

	.zeco-participate-bid .box-section img{
		width: 80%;
	}
	.zeco-participate-bid .zeco-box-option{
		font-size: 0.8rem;
		color: darken($color-grey, 20%);
	}
	.zeco-participate-bid .zeco-box-option .option-left{
		margin-top: 2px;
	}
	.zeco-participate-bid .option-left{
		float: left;
		width: 60%;
		text-align: left;
	}
	.zeco-participate-bid .option-right{
		float: right;
		width: 40%;
		text-align: right;
	}
	.zeco-participate-bid .option-left>div, 
	.zeco-participate-bid .option-left b, 
	.zeco-participate-bid .option-left .address-detail{
		float: left;
		margin-top: 1px;
	}
	.zeco-participate-bid .option-left, 
	...
	.zeco-participate-bid .icon-tag-bid{
		background-color: #fcc854;
		color: $bid-color;
	}
	.zeco-participate-bid .icon-tag-recruit{
		background-color: #0266bb;
		color: $color-white;
	}
	.zeco-participate-bid .option-addresses{
		max-width: 70%;
		padding-left: 5px;
	}
	.zeco-participate-bid .address-detail{
		max-width: 90%;
	}
	.zeco-participate-bid .option-status{
		padding: 3px;
		font-size: 0.9rem;
		color: $bid-color;
	}

	.zeco-participate-bid .zeco-box-assistant{
		line-height: 1rem;
		vertical-align: sub;
	}
	.zeco-participate-bid .zeco-box-assistant .option-left, 
	.zeco-participate-bid .zeco-box-assistant .option-right{
		width: 50%;
	}
	.zeco-participate-bid .zeco-box-assistant .option-left{
		margin-top: 1px;
	}
	.zeco-participate-bid .assistant-time{
		font-size: 0.7rem;
	...
	.zeco-participate-bid .assistant-limit{
		font-size: 0.8rem;
		color: #ef7979;
	}

	// 加载控件
	.zeco-participate-bid .zeco-info-loading{
	    height: 50px;
	    line-height: 50px;
	    position: relative;
	}
	.zeco-participate-bid .zeco-loading-cover{
	    position: absolute;
	    left: 50%;
	    margin-left: -44.67px;
	}
	.zeco-participate-bid .zeco-loading-cover .loading-animation{
		float: left;
	    margin-top: 11px;
	}
	.zeco-participate-bid .zeco-loading-cover .loading-name{
		margin-left: 10px;
		float: left;
	}
</style>