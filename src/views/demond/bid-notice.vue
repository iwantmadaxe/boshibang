	...
	<div class="zeco-bid-notice">
		<ul v-infinite-scroll="loadMore()" infinite-scroll-disabled="loading" infinite-scroll-distance="50" v-show="list.length">
			<li class="list-box-content" v-for="listItem in list">
				<box-section :head-title="listItem.title" :sub-title="listItem.range" v-link="{name: 'Bid', params: {bid_no: listItem.id}}">
					<img :src="companyLogo" slot="img" width="100%">
					<div class="addition-time" slot="more">
						<div class="publishment-time">
							<span class="publishment-title">发布时间:</span>
							<span class="publishment-content">{{ listItem.updatedAt }}</span>
						</div>
						<div class="limitation-time">
							<span class="limitation-title">截止时间:</span>
		...
		methods: {
			loadMore () {
				...
				this.$http.get(apis.urls.companyLatestRecruit, {params: {id: this.companyId, page: page}}).then((response) => {
					let listTpl = apis.pures.pureCompanyLatestBid(response.body);
			...
			loadingData () {
				this.loading = true;
				// 获取列表数据
				this.$http.get(apis.urls.companyLatestBid, {params: {id: this.companyId, page: 1}}).then((response) => {
					let tpl = apis.pures.pureCompanyLatestBid(response.body);
					this.list = tpl.items;
...
<style lang="scss">
	...
	.zeco-bid-notice ul>li:not(:last-child){
		@include border-bottom($border-color);
	}
	.zeco-bid-notice .addition-time{
		font-size: 0.8rem;
		color: darken($color-grey, 20%);
	}
	.zeco-bid-notice .addition-time .publishment-content,
	.zeco-bid-notice .addition-time .limitation-content{
		color: #666;
	}

	.zeco-bid-notice .zeco-info-loading{
	    height: 50px;
	    line-height: 50px;
	    position: relative;
	}
	.zeco-bid-notice .zeco-loading-cover{
	    position: absolute;
	    left: 50%;
	    margin-left: -44.67px;
	}
	.zeco-bid-notice .zeco-loading-cover .loading-animation{
		float: left;
	    margin-top: 11px;
	}
	.zeco-bid-notice .zeco-loading-cover .loading-name{
		margin-left: 10px;
		float: left;
	}
</style>