<template>
	<div class="zeco-recruit">
			...
			<div class="company-summary" @click="goCompany(details.publish.companyType, details.publish.companyId)">
				<div class="list-box-content">
					<box-section :head-title="details.publish.companyName" sub-title="关注我们可获得更多采购需求>>">
						<img slot="img" :src="details.publish.logo" width="100">
					</box-section>
				</div>
			</div>
		</section>
		...
		<section class="recruit-project-bar">
	...
	export default {
		...
		route: {
			data (transition) {
				...
				this.$http.get(apis.urls.recruitdetail, {params: {id: this.recruitNo}}).then((response) => {
					Indicator.close();
					this.details = apis.pures.pureRecruitDetail(response.body);
				}, (response) => {
					Indicator.close();
					apis.errors.errorRecruitDetail(response, this);
				});
			}
		},
		vuex: {
		...
		methods: {
			upSign () {
				...
				let options = {
					params: {recruit_id: this.recruitNo},
...
<style lang="scss">
	...
	// 控制条
	.zeco-recruit .recruit-project-bar{
		height: 55px;
	}
	.zeco-recruit .recruit-project-bar .mint-tab-item .mint-tab-item-label{
		line-height: 41px;
		@include border-right($border-color);
		font-size: 0.9rem;
	}
	.zeco-recruit .recruit-project-bar .mint-tab-item.is-selected:first-child{
		color: $color-white;
		background-color: #0266bb;
	}
	.zeco-recruit .recruit-project-bar .mint-tab-item.is-selected:last-child{
		color: $bid-color;
		background-color: $main-background-color;
	}

	// 条件
	.zeco-recruit .recruit-project-condition{
		margin-top: 10px;
		@include border-top($border-color);
	}

	// 标题栏目
	.zeco-recruit .recruit-project-info{
		background-color: $color-white;
		padding: 20px 0 0 10px;
		@include border-bottom($border-color);
	}
	.zeco-recruit .project-summary{
		padding-right: 10px;
		@include border-bottom($border-color);
	}
	.zeco-recruit .option-left{
		float: left;
		text-align: left;
	}
	.zeco-recruit .option-right{
		float: right;
		text-align: right;
	}

	// 主标题
	.zeco-recruit .project-name{
		color: $color-black;
		font-size: 1rem;
		text-align: left;
		padding-bottom: 10px;
	...
	// 副标题
	.zeco-recruit .project-qualification{
		text-align: left;
		font-size: 0.9rem;
	...
	// 基础信息
	.zeco-recruit .project-option{
		padding-bottom: 10px;
	}
	.zeco-recruit .project-summary .project-option span,
	.zeco-recruit .project-summary .project-option b{
		display: inline-block;
		vertical-align: middle;
		font-size: 0.9rem;
	}
	.zeco-recruit .icon-tag-recruit{
		color: $color-white;
		background-color: $bid-background-color;
	}
	.zeco-recruit .option-addresses{
		color: darken($color-grey, 20%);
	}
	.zeco-recruit .option-money{
		color: $bid-color;
	}

	// 限制条件
	.zeco-recruit .project-summary .project-assistant span,
	.zeco-recruit .project-summary .project-assistant b{
		display: inline-block;
		vertical-align: middle;
		font-size: 0.8rem;
	}
	...
	// 人数和时间
	.zeco-recruit .project-assistant{
		padding-bottom: 10px;
	}
	.zeco-recruit .assistant-watch-number{
		padding-left: 5px;
	}
	.zeco-recruit .assistant-watch-number .watch-number{
		margin-left: -3px;
	}
	.zeco-recruit .assistant-watch-number .icon-watch{
		width: 11px;
		height: 8px;
		display: inline-block;
	...
	// 截止报名时间
	.zeco-recruit .assistant-limit{
		color: $limit-color;
	}

	// 公司介绍
	.zeco-recruit .company-summary{
		padding-right: 10px;
	}
	.zeco-recruit .company-summary .box-pic{
		text-align: left;
	}
	.zeco-recruit .company-summary .box-content{
		text-align: right;
	}
	.zeco-recruit .box-section{
		margin: 0;
	}
	.zeco-recruit .box-section:before{
		border-bottom: 0;
	}
</style>