<template>
	<div class="supplier-list-info">
		<ul v-infinite-scroll="loadMore()" infinite-scroll-disabled="loading" infinite-scroll-distance="50">
			<li class="list-box-content" v-for="listItem in list" v-link="{name: 'Supply', params: {supply_no: listItem.companyId}}">
				<box-section :head-title="listItem.companyName">
					<img :src="listItem.logo" slot="img" class="recruit-logo" width="80%">
					<div class="manage-more" slot="more">
						<div class="manage-base cl-fx">
							<div class="manage-left">
								<span class="base-title">注册资本:</span>
								<span class="base-content">{{listItem.companyMoney}}万以上</span>
							</div>
						...
						<div class="manage-ability">
							<span class="ability-content">{{listItem.companyAddress}}</span>
						</div>
					</div>
				</box-section>
			</li>
	...
	export default {
		...
		methods: {
			loadMore () {
				...
				this.$http.get(apis.urls.provider, options).then((response) => {
					let listTpl = apis.pures.pureDeveloperSupplierList(response.body);
...
<style lang="scss">
	...
	.supplier-list-info ul>li:not(:last-child){
		@include border-bottom($border-color);
	}
	.supplier-list-info .manage-base,
	.supplier-list-info .manage-ability{
		margin-top: 5px;
		font-size: 0.9rem;
		color: darken($color-grey, 20%);
	}
	.supplier-list-info .manage-base .manage-left{
		float: left;
		text-align: left;
	}
	.supplier-list-info .manage-base .manage-right{
		float: right;
		text-align: right;
	}
	.supplier-list-info .manage-base .base-title{
		color: #666;
	}
	.supplier-list-info .manage-base .base-content{
		color: $bid-color;
	}
	.supplier-list-info .manage-base .manage-right{
		max-width: 35%;
	}
	.supplier-list-info .manage-base .base-address{
	...
	// 加载控件
	.supplier-list-info .zeco-info-loading{
	    height: 50px;
	    line-height: 50px;
	    position: relative;
	}
	.supplier-list-info .zeco-loading-cover{
	    position: absolute;
	    left: 50%;
	    margin-left: -44.67px;
	}
	.supplier-list-info .zeco-loading-cover .loading-animation{
		float: left;
	    margin-top: 11px;
	}
	.supplier-list-info .zeco-loading-cover .loading-name{
		margin-left: 10px;
		float: left;
	}
</style>