<template>
	<div class="zeco-win-bid-list">
		<section class="win-project-manage">
			<ul>
				<li class="list-box-content" v-for="item in winBid">
...
<script>
	...
	import { winBid } from '../../../vuex/getters.js';

	export default {
		name: 'zeco-win-bid-list',
		vuex: {
			getters: {
				winBid
			}
...
<style lang="scss">
	...
	// 用户信息布局
	.zeco-win-bid-list ul>li:not(:last-child){
		@include border-bottom($border-color);
	}
	.zeco-win-bid-list .company-info{
		margin-top: 3px;
	}
	.zeco-win-bid-list .company-info .company-address{
		display: inline-block;
		vertical-align: middle;
		max-width: 65%;
	}
	.zeco-win-bid-list .company-info .info-left{
		float: left;
		text-align: left;
		max-width: 70%;
		overflow: hidden;
	...
	.zeco-win-bid-list .company-info .info-right{
		float: right;
		text-align: right;
		max-width: 30%;
		overflow: hidden;
	...
	// 选项框修正
	.zeco-win-bid-list .company-bars{
		margin-top: 15px;
	}
</style>