...
export default {
	...
	computed: {
		heightAvail () {
			let tplHeight = 0;
			if (!this.height && this.lockX) {
				tplHeight = document.documentElement.clientHeight;
				this.reset();
			}
			if (this.height && this.height.indexOf('-') === 0) {
				tplHeight = document.documentElement.clientHeight + parseInt(this.height);
			}
